...
namespace util {
    ...
    inline auto dump(std::ostream &output, const std::uint8_t *buffer, std::size_t length, int radix = 16, int entries_line = 8) -> void {
        ...
        output << std::setw(static_cast<int>(max_address_chars + static_cast<std::streamsize>(2))) << "" << std::setw(static_cast<int>(1));
        ...
        if (last_line_entry != 0) {
            ...
            output << std::setw(static_cast<int>((static_cast<std::streamsize>(entries_line) - last_line_entry) * (static_cast<std::streamsize>(entry_size) + 1))) << "" << std::setw(1) << " " << text << "\n";
        }
    }
